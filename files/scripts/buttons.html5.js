	...
	action: function ( e, dt, button, config ) {
		...
		if( config.multitables!==undefined && $.isPlainObject(config.multitables) ){
			for(var key in config.multitables){
				...
				tabledataInfo[key]=$('#'+key).DataTable().buttons.exportInfo( config );
				counter++;
			...
			tabledataInfo[dt.table().node().id]=dt.buttons.exportInfo( config );
		...
		var addHdrBodyFtrWidth = function (exportInfo,data,rels,relsGet) {
			...
			if ( exportInfo.title ) {
				addRow( [exportInfo.title], rels, relsGet, rowPos );
				mergeCells( rowPos, data.header.length-1,rels );
			}

			if ( exportInfo.messageTop ) {
				addRow( [exportInfo.messageTop], rels, relsGet, rowPos );
				mergeCells( rowPos, data.header.length-1,rels );
			}
			...
			if ( exportInfo.messageBottom ) {
				addRow( [exportInfo.messageBottom], rels, relsGet, rowPos );
				mergeCells( rowPos, data.header.length-1,rels );
			}
		...
		for(key in tabledata){
			if(counter==0) fileName = tabledataInfo[key].filename;
			counter++;
			relsarray[key] = getXml('xl/worksheets/sheet1.xml');
			var relsGet = relsarray[key].getElementsByTagName( "sheetData" )[0];
			addHdrBodyFtrWidth(tabledataInfo[key], tabledata[key], relsarray[key], relsGet);
